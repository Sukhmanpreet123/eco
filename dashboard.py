"""
EcoTrace v3 — Streamlit Dashboard (no-flicker version)

Root cause of flicker: while True + st.rerun() wipes and rebuilds
the entire page every 5 seconds. The fix:
  1. Use streamlit-autorefresh which only increments a counter
     and triggers rerun WITHOUT clearing static layout elements.
  2. Remove the while True loop entirely.
  3. Move all static elements (title, sidebar, tabs) OUTSIDE any loop.
  4. Only the data-fetching and metric rendering is inside the auto-refresh.
"""
...
# Install once: pip install streamlit-autorefresh
try:
...
# ── Inject CSS to prevent layout shift during refresh ─────────
st.markdown("""
<style>
/* Prevent the flash of empty content during rerun */
.stMetric, .stPlotlyChart, .stDataFrame, .stAlert {
...
# ── SESSION STATE INIT (only runs once) ──────────────────────
...
# ── SIDEBAR (static — rendered once, never inside a loop) ─────
with st.sidebar:
    ...
    # Reset history when device changes
    ...
    st.markdown("---")
    # Project totals — fetched once per page load (not per tick)
...
# This is the key fix: st_autorefresh triggers a rerun every N ms
# WITHOUT wiping the layout. The counter just increments.
# If streamlit-autorefresh is not installed, we fall back gracefully.
if HAS_AUTOREFRESH:
    tick = st_autorefresh(interval=REFRESH_MS, key="live_refresh")
else:
    # Fallback: show install instruction once, no auto-refresh
    st.warning(
        "⚠️ Install `streamlit-autorefresh` for smooth live updates: "
        "`pip install streamlit-autorefresh`")
    tick = 0


# ── PAGE TABS (static structure — never inside a loop) ────────
...
# ══════════════════════════════════════════════════════════════
# TAB 1 — LIVE MONITOR
# No while loop. No st.rerun(). No time.sleep().
# Data is fetched fresh on every auto-refresh tick.
# ══════════════════════════════════════════════════════════════
with tab_live:
    st.title("🌱 EcoTrace — Real-Time AI Carbon Governance")

    # ── Fetch all live data ───────────────────────────────────
    res    = api("/predict",        params={"session_id": target})
    budget = api("/budget_check",   params={"session_id": target,
                                            "budget_g":   BUDGET_G})
    anom   = api("/anomalies",      params={"session_id": target})
    cmp    = api("/fingerprint/compare", params={"session_id": target})
    ...
    # DNA match from live power buffer
    dna_r = {}
    ...
    c1.metric("Current power",
              fmt_val(curr_w, "W"),
              help="Average wattage in last window")
    ...
    # Last updated timestamp — subtle, not flashy
    st.caption(
        f"🔗 Connected · {target} · "
        ...
        if proj_co2_live >= 1.0 and bco2 >= 1.0:
            ...
            if pct > 20:
                st.warning(
                    ...
                    f"(...{short_id}: {bco2}g → {acc_str}, "
                    f"Grade {bgrade}). Reduce epochs or batch size.")
            else:
                st.success(
                    f"📊 On track — similar to ...{short_id} "
                    f"({bco2}g CO₂, {acc_str}, Grade {bgrade}).")
        else:
            st.info("📊 Past runs found — call end_session() "
    ...
    if dna_r.get("match"):
        ...
        st.info(
            f"🧬 Carbon DNA match "
            f"({round(m['similarity']*100,1)}% similar) — "
    ...
    if shap.get("co2_fair_g") is not None:
        ...
        if saved > 0:
            st.success(
                ...
                f"{shap['co2_fair_g']}g "
                f"(saved {shap['co2_saved_g']}g vs naive attribution)")

    # ── ROW 3: Heartbeat chart + Carbon debt ──────────────────
    ...
    with col_chart:
        ...
        fig.update_layout(
            ...
            # Key: uirevision keeps the chart from re-animating on refresh
            uirevision="ecotrace_heartbeat")

        st.plotly_chart(fig, use_container_width=True)

        if anomalies:
            st.caption(
                f"⚠️ {len(anomalies)} anomaly event(s) — "
                f"wasted carbon logged.")
    ...
    if fp_list:
        ...
        if "timestamp" in fp_df.columns and "total_co2_g" in fp_df.columns:
            fig_t = go.Figure(go.Scatter(
                ...
                mode="lines+markers",
                line=dict(color="#3498db"), name="CO₂ per run"))
...
with tab_sla:
    ...
    with st.form("sla_form"):
        ...
        sla_max_co2 = st.number_input("Max CO₂ (g)", 1.0, 10000.0,
                                       50.0, step=1.0)
        sla_min_acc = st.number_input("Min accuracy (0–1)", 0.0, 1.0,
                                       0.90, step=0.01)
...
with tab_behavior:
    ...
    st.caption("Analyses training habits to find carbon waste patterns.")
    if st.button("🔄 Refresh report"):
        st.rerun()

    beh_rid = st.text_input("Researcher ID (leave blank for all)")
    beh     = api("/behavior",
                  params={"researcher_id": beh_rid}
                  if beh_rid else {})
    ...
    if beh.get("total_runs"):
        ...
        with cb:
            st.metric("Late-night runs (10 PM–5 AM)",
                      ...
                      delta_color="inverse")
            st.caption("Night runs often have higher waste %.")
